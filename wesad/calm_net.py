...
class BranchNetMultiTask(nn.Module):
    def __init__(
        ...
        # self.d_cnn_liner = RNNEncoder(in_channel, emb_size)

        self.branching = Branching(
            input_size=emb_size, # +9 when add covariates
    ...
    def forward(self, x, subjects, cov):
        # x: N x L x C
        # conv: N x C
        N, L, C = x.shape
        _, Co = cov.shape
        x = x.unsqueeze(dim=1) # become (N, 1, L, C)
        out = self.d_cnn_liner(x) # (N, 1, L, E)
        # out = torch.cat((out, cov.unsqueeze(1).expand(N, L, Co)), dim=2)

        # branching
        out = self.branching(out)
    ...
    def fit(
        ...
        for e in tqdm(range(epochs)):
            ...
            for data_pack in tqdm(train_loader):
                iteration += 1
                # forward
                out = self(data_pack["signals"], data_pack["subjects"], data_pack["covs"])
